...
// @desc     creat or update user profil
// @access   Private
router.post(
  "/",
  [
...
router.get("/", async (req, res) => {
  try {
    ...
    console.error(error.message);
    res.status(500).send("Error get all profile");
  }
});
...
router.get("/user/:user_id", async (req, res) => {
  try {
    ...
    if (error.kind == "ObjectId") {
      return res.status(400).json({ msg: "Not found profile for this user" });
    }
...
// @desc     Delete  profile ,user & post user completly
// @access   Private
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error(error.message);
      res.status(500).json({ msg: "Exp errors" });
    }
  }
);
